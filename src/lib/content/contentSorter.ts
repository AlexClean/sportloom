...
export function getLatestContentByType(contentType: string, limit: number = 3, excludeSlug?: string) {
  return META_LIST?.filter(entry => entry.contentType === contentType && entry.slug !== excludeSlug)
